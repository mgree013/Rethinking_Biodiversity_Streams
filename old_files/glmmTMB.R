...
mod3<-glmmTMB(betas.LCBD~Head.river.dist*River.dist.lake-River.dist.lake+ (1|O.NET),family=beta_family(),data=data)
...
mod1<-glmer(betas.LCBD~River.dist.lake+ (1|O.NET),family=gaussian(), data=dd_specie)
mod2<-glmer(betas.LCBD~Head.river.dist+ (1|O.NET),family=gaussian(),data=dd_specie)
mod3<-glmer(betas.LCBD~Head.river.dist*River.dist.lake+ (1|O.NET),family=gaussian(),data=dd_specie)
null<-glmer(betas.LCBD~1+ (1|O.NET),family=gaussian(),data=dd_specie)
reported.table2 <- bbmle::AICtab(mod1,mod2,mod3,null,weights = TRUE, sort = FALSE)
...
mod3<-glmmTMB(N1~River.dist.lake*Head.river.dist-River.dist.lake+ (1|O.NET),family=gaussian(),data=dd_specie)
...
mod5<-glmmTMB(betas.LCBD~Spatial*Com.Size.Gradient-Spatial+ (1|O.NET),family=beta_family(), data=all_big_dat)
mod6<-glmmTMB(betas.LCBD~E_PC1*Com.Size.Gradient-E_PC1+ (1|O.NET),family=beta_family(), data=all_big_dat)
...
mod4<-glmmTMB(N1~Spatial*E_PC1-Spatial+ (1|O.NET),family=gaussian(),data=all_big_dat)
mod5<-glmmTMB(N1~Spatial*Com.Size.Gradient-Spatial+ (1|O.NET),family=gaussian(), data=all_big_dat)
mod6<-glmmTMB(N1~E_PC1*Com.Size.Gradient-E_PC1+ (1|O.NET),family=gaussian(), data=all_big_dat)
...
reported.table2 <- bbmle::AICtab(mod1,mod2,mod3,mod4,mod5,mod6,mod7, null,weights = TRUE, sort = F)
